...
Output: ranked, enriched drug candidates for all downstream agents
    ...
    from agent2.discover import discover_candidates
    candidates = discover_candidates("glioblastoma", data_dir="./data")

    # Each candidate has everything downstream agents need:
    for c in candidates:
        c.drug_name       # "Metformin"       → Agent 1, 3, 4
        c.chembl_id       # "CHEMBL1431"      → Agent 1
        c.smiles          # "CN(C)C(=N)..."   → Agent 5, 6
        c.kg_percentile   # 97.3              → Orchestrator
        c.status          # "dropped"         → Orchestrator
"""
...
class _DBEnricher:
    """Loads dropped_drugs.db into memory for fast cross-referencing."""
    ...
    def __init__(self, db_path: str):
        ...
        conn.close()
        self._loaded = True
        logger.info("DB enricher: %d drugs, %d withdrawn",
                     len(self._by_name), len(self._withdrawn))
...
def discover_candidates(
    ...
    Args:
        disease:         "glioblastoma", "alzheimer", etc.
        data_dir:        Path to your data/ folder (with embeddings/, database/)
        top_k:           How many top KG hits to scan before filtering
        max_candidates:  Maximum candidates to return
        min_percentile:  Minimum KG percentile to include
        include_novel:   Include compounds not in dropped_drugs.db
        require_smiles:  Skip candidates without SMILES
    """
